...
import (
	...
	"hop.top/kit/go/integrations/repohost/github/mock"
)

func openHost(t *testing.T) repohost.MutableHost {
	t.Helper()
	t.Cleanup(mock.Reset)
	host, err := repohost.Open(context.Background(), repohost.Config{Provider: "github-mock"})
	if err != nil {
		t.Fatalf("Open: %v", err)
	}
	return host
...
func TestMock_SetCommit(t *testing.T) {
	...
	// Other SHAs still default to baseline.
...
func TestMock_SetError(t *testing.T) {
	...
	// Clearing the error knob (nil) restores defaults.
...
func TestMock_ProviderMismatch(t *testing.T) {
	_, err := repohost.Open(context.Background(), repohost.Config{Provider: "github-mock"})
	if err != nil {
		t.Fatalf("expected success for github-mock: %v", err)
	}
}
