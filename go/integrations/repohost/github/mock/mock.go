...
func init() {
	repohost.RegisterDriver("github-mock", openMock)
}

// openMock is the [repohost.Opener] for the github-mock provider.
func openMock(cfg repohost.Config) (repohost.MutableHost, error) {
	if cfg.Provider != "github-mock" {
		return nil, fmt.Errorf("github-mock: provider mismatch: got %q", cfg.Provider)
	}
	return &Host{}, nil
}
