...
func init() {
	repohost.RegisterDriver("bitbucket-mock", openMock)
}

// openMock is the [repohost.Opener] for the bitbucket-mock provider.
func openMock(cfg repohost.Config) (repohost.MutableHost, error) {
	if cfg.Provider != "bitbucket-mock" {
		return nil, fmt.Errorf("bitbucket-mock: provider mismatch: got %q", cfg.Provider)
	}
	return &Host{}, nil
}
