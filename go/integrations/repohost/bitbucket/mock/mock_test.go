...
import (
	...
	"hop.top/kit/go/integrations/repohost/bitbucket/mock"
)

func openHost(t *testing.T) repohost.MutableHost {
	t.Helper()
	t.Cleanup(mock.Reset)
	host, err := repohost.Open(context.Background(), repohost.Config{Provider: "bitbucket-mock"})
	if err != nil {
		t.Fatalf("Open: %v", err)
	}
	return host
...
func TestMock_ProviderMismatch(t *testing.T) {
	_, err := repohost.Open(context.Background(), repohost.Config{Provider: "bitbucket-mock"})
	if err != nil {
		t.Fatalf("expected success for bitbucket-mock: %v", err)
	}
}
