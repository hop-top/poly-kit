...
import (
	...
	_ "hop.top/kit/go/integrations/repohost/gitee"
)
...
const fixturePR = `{
  ...
  "body": "body text",
  "state": "open",
  "html_url": "https://gitee.com/foo/bar/pulls/1",
...
const fixturePRMerged = `{
  ...
  "title": "Old feature",
  "state": "merged",
  "html_url": "https://gitee.com/foo/bar/pulls/2",
  "user": {"login": "bob"},
  "head": {"ref": "feature2"},
  "base": {"ref": "main"},
  "labels": [],
...
const fixturePRClosed = `{
  ...
  "title": "Abandoned",
  "state": "closed",
  "html_url": "https://gitee.com/foo/bar/pulls/3",
  "user": {"login": "carol"},
  "head": {"ref": "feature3"},
  "base": {"ref": "main"},
  "labels": [],
...
const fixtureGTIssue = `{
  "id": 100,
  "number": "10",
  "title": "Bug found",
  "state": "open",
  "html_url": "https://gitee.com/foo/bar/issues/10",
  "user": {"login": "alice"},
...
const fixtureGTCommit = `{
  "sha": "abc123",
  "html_url": "https://gitee.com/foo/bar/commit/abc123",
  "commit": {
...
const fixtureGTComment = `{
  "id": 999,
  "body": "looks good",
  "user": {"login": "alice"},
  "html_url": "https://gitee.com/foo/bar/issues/1#note_999",
  "created_at": "2026-01-01T00:00:00Z"
}`
...
func TestHost_ListPullRequests_StateFilters(t *testing.T) {
	cases := []struct {
		...
		{"open-zero", repohost.Filter{}, "open"},
...
func TestHost_ListPullRequests_Limit(t *testing.T) {
	host, _ := newServer(t, func(w http.ResponseWriter, r *http.Request) {
		// gitee per_page param name varies; the encoder writes per_page.
		// Test that go-scm passed _some_ limit-like param.
		fmt.Fprintf(w, "[%s, %s]", fixturePR, fixturePRMerged)
	})
...
func TestHost_ListIssues(t *testing.T) {
	host, _ := newServer(t, func(w http.ResponseWriter, r *http.Request) {
		fmt.Fprintf(w, "[%s]", fixtureGTIssue)
	})
	...
	if issues[0].Title != "Bug found" || issues[0].State != "open" {
		t.Errorf("issue fields wrong: %+v", issues[0])
	}
}
...
func TestOpen_TokenFromEnv(t *testing.T) {
	t.Setenv("GITEE_TOKEN", "envtoken")
	host, err := repohost.Open(context.Background(), repohost.Config{Provider: "gitee"})
	if err != nil {
		t.Fatalf("Open: %v", err)
	}
	if host == nil {
...
func TestOpen_ProviderMismatch(t *testing.T) {
	_, err := repohost.Open(context.Background(), repohost.Config{Provider: "gitee-bogus"})
	if err == nil {
