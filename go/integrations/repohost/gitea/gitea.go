...
// Host is the Gitea driver implementation of [repohost.MutableHost],
...
// ListIssues returns issues matching f for repo ("owner/name"). Note:
// go-scm's Gitea driver does not surface the issue's pull_request
// field, so the kit driver cannot client-side filter PRs out of the
// /issues endpoint; callers that need that distinction should use
// ListPullRequests + ListIssues separately. Author and label filters
...
func (h *Host) GetCommit(ctx context.Context, repo, sha string) (repohost.Commit, error) {
	if _, _, err := splitRepo(repo); err != nil {
		return repohost.Commit{}, fmt.Errorf("gitea: get commit: %w", err)
	}
...
func (h *Host) GetRepo(ctx context.Context, repo string) (repohost.Repo, error) {
	if _, _, err := splitRepo(repo); err != nil {
		return repohost.Repo{}, fmt.Errorf("gitea: get repo: %w", err)
	}
...
// number. Gitea uses a single endpoint for both PR and issue
// comments; go-scm's gitea pull-comment routes via review surface so
// the kit driver dispatches PR-first then falls back to the issue
// endpoint on ErrNotSupported / 404 to keep behavior uniform.
func (h *Host) PostComment(ctx context.Context, repo string, number int, body string) (repohost.Comment, error) {
	if body == "" {
		return repohost.Comment{}, errors.New("gitea: comment body must not be empty")
	}
	if _, _, err := splitRepo(repo); err != nil {
		return repohost.Comment{}, fmt.Errorf("gitea: post comment: %w", err)
	}
...
func wrapErr(op string, err error, resp *scm.Response, sentinel error) error {
	if is404(resp) || errors.Is(err, scm.ErrNotFound) {
		return fmt.Errorf("gitea: %s: %w", op, sentinel)
	}
	return fmt.Errorf("gitea: %s: %w", op, err)
}
...
func mapPullRequest(pr *scm.PullRequest) repohost.PullRequest {
	...
	out.Raw["has_merged"] = pr.Merged
	out.Raw["state_raw"] = stateRaw(pr.Closed, pr.Merged)
	return out
}

// mapIssue is bound to *Host so it can synthesize Issue.URL when
// go-scm leaves it empty (gitea's convertIssue is unfinished upstream).
func (h *Host) mapIssue(repo string, is *scm.Issue) repohost.Issue {
	...
	out.URL = is.Link
	if out.URL == "" && repo != "" && is.Number != 0 {
		out.URL = fmt.Sprintf("%s/%s/issues/%d", h.baseURL, repo, is.Number)
	}
