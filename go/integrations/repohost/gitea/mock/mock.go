...
func init() {
	repohost.RegisterDriver("gitea-mock", openMock)
}

// openMock is the [repohost.Opener] for the gitea-mock provider.
func openMock(cfg repohost.Config) (repohost.MutableHost, error) {
	if cfg.Provider != "gitea-mock" {
		return nil, fmt.Errorf("gitea-mock: provider mismatch: got %q", cfg.Provider)
	}
	return &Host{}, nil
}
