...
import (
	...
	"hop.top/kit/go/integrations/repohost/gitea/mock"
)

func openHost(t *testing.T) repohost.MutableHost {
	t.Helper()
	t.Cleanup(mock.Reset)
	host, err := repohost.Open(context.Background(), repohost.Config{Provider: "gitea-mock"})
	if err != nil {
		t.Fatalf("Open: %v", err)
	}
	return host
...
func TestMock_ProviderMismatch(t *testing.T) {
	_, err := repohost.Open(context.Background(), repohost.Config{Provider: "gitea-mock"})
	if err != nil {
		t.Fatalf("expected success for gitea-mock: %v", err)
	}
}
