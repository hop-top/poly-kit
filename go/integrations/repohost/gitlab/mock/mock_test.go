...
import (
	...
	"hop.top/kit/go/integrations/repohost/gitlab/mock"
)

func openHost(t *testing.T) repohost.MutableHost {
	t.Helper()
	t.Cleanup(mock.Reset)
	host, err := repohost.Open(context.Background(), repohost.Config{Provider: "gitlab-mock"})
	if err != nil {
		t.Fatalf("Open: %v", err)
	}
	return host
...
func TestMock_ProviderMismatch(t *testing.T) {
	_, err := repohost.Open(context.Background(), repohost.Config{Provider: "gitlab-mock"})
	if err != nil {
		t.Fatalf("expected success for gitlab-mock: %v", err)
	}
}
