...
func init() {
	repohost.RegisterDriver("gitlab-mock", openMock)
}

// openMock is the [repohost.Opener] for the gitlab-mock provider.
func openMock(cfg repohost.Config) (repohost.MutableHost, error) {
	if cfg.Provider != "gitlab-mock" {
		return nil, fmt.Errorf("gitlab-mock: provider mismatch: got %q", cfg.Provider)
	}
	return &Host{}, nil
}
