// Package envelope holds kit's structured-error envelope: the Error
// shape, the standard codes and exit codes, the transience vocabulary,
// and the renderer that serializes them.
//
// It is deliberately a leaf. The envelope is the one part of the output
// contract that library code returns — a runtime package that denies an
// operation has to name a code and an exit code — while table styling,
// column projection and TTY detection belong to the renderers in the
// parent package. Go's unit of dependency is the package, so leaving the
// two in one package forced every consumer of the envelope to link the
// terminal-UI stack. Anything imported here must stay stdlib plus the
// YAML encoder.
//
// Adopters keep importing hop.top/kit/go/console/output, which aliases
// every name declared here.
//
...
// Format keys RenderError switches on. They repeat the values of the
// parent package's Format constants rather than importing them: those
// are declared beside the table renderer, and importing them back would
// restore the very edge this package exists to cut. The parent aliases
// Format itself, so the two cannot drift in type — only these two
// literals are duplicated, and they are frozen wire values.
const (
	formatJSON = "json"
	formatYAML = "yaml"
)
