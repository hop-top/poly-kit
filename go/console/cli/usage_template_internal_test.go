...
// usageTemplate departs from cobra's default only for empty sections and
// hidden commands. Where every group lists a visible command it must
// render byte for byte what cobra renders.
func TestUsageTemplate_MatchesCobraWhenNoSectionIsEmpty(t *testing.T) {
	build := func() *cobra.Command {
		...
		root.AddGroup(&cobra.Group{ID: "a", Title: "GROUP A"})
		root.AddCommand(
			&cobra.Command{Use: "one", Short: "First", GroupID: "a", Run: run},
			&cobra.Command{Use: "two", Short: "Second", Run: run, Aliases: []string{"2"}},
		)
		sub := &cobra.Command{Use: "sub", Short: "Sub"}
		sub.AddCommand(&cobra.Command{Use: "leaf", Short: "Leaf", Run: run})
		root.AddCommand(sub)
