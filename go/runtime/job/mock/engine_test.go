...
func newEngine() *mock.Engine {
	return mock.New(job.WithNowFunc(fixedNow))
}
...
func TestFailRetryThenComplete(t *testing.T) {
	now := fixedNow()
	e := mock.New(job.WithNowFunc(func() time.Time { return now }))
	ctx := context.Background()
...
func TestRunOne(t *testing.T) {
	t.Setenv(job.EnvRunOneDisable, "")
	ctx := context.Background()
	e := newEngine()

	e.Enqueue(ctx, job.EnqueueOpts{Queue: "q", Type: "test"})

	var handled bool
	handlers := job.HandlerMap{
		"test": func(_ context.Context, j job.Job) error {
			handled = true
			return nil
		},
	}

	err := job.RunOne(ctx, e, "q", "w", handlers)
	require.NoError(t, err)
