...
/// Resolve `<XDG_CONFIG_HOME>/kit/telemetry.yaml`, honoring the
/// `XDG_CONFIG_HOME` env var with a `$HOME/.config` fallback.
pub fn consent_path() -> PathBuf {
    let base = std::env::var_os("XDG_CONFIG_HOME")
        .map(PathBuf::from)
        .unwrap_or_else(|| dirs::home_dir().unwrap_or_default().join(".config"));
    base.join("kit").join("telemetry.yaml")
}
...
pub fn load_consent() -> Consent {
    ...
    let Ok(parsed) = serde_yaml::from_str::<Wrapper>(&content) else {
        return Consent::denied();
    };
    let Some(c) = parsed.telemetry.and_then(|t| t.consent) else {
        return Consent::denied();
    };
