# Shared template blueprints

> Reference for the files under
> [`templates/shared/`](../../templates/shared/README.md): the
> `tool-versions.toml` SOT, gitignore and gitattributes composition, the
> managed-block library, every `emit-*.sh` emitter, and the `--services`
> catalog.

## Who this is for

Template authors changing what `templates/scaffold.sh` and `kit init`
write into a scaffolded project. Adopters running the scaffold want
[`kit-init.md`](../adopters/guides/kit-init.md) and
[`author-a-template.md`](../adopters/guides/author-a-template.md)
instead.

`kit init` embeds a byte-identical mirror of these files under
`cmd/kit/init/managed_assets/`, kept in sync by a pre-commit hook, so a
refresh works with just the `kit` binary on `$PATH`. Changing anything
here means re-running that sync.
...
| `# kit-managed: telemetry` | managed | default `otel-collector` + `jaeger`; image tags pinned in [`emit-docker-compose.sh`](../../templates/shared/emit-docker-compose.sh) |
