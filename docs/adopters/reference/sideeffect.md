...
> Dry-run and test seam for kit-based tools
> (`hop.top/kit/go/runtime/sideeffect`). Interfaces, the three
> implementations, the `--dry-run` flag and its policy table, adoption and
> opt-out, migration, bus auto-tagging, and what dry-run does and does not
> guarantee.

## Who this is for

Go authors whose commands write files, make mutating HTTP calls, publish
events or spawn subprocesses, and who want `--dry-run` and unit tests to
intercept all four without duplicating logic.
...
The `kit/side-effect` convention (sections 3.5 and 3.6 of the CLI
conventions) is
...
because the wrapper does not clone-and-augment because subscriber pipelines
