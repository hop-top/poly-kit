...
> Scope operating-mode primitive for kit-based tools
> (`hop.top/kit/go/core/stage`). Stage table, `State` shape, event topics,
> `projects.yaml` v2 schema, wiring example.

## Who this is for

Go authors declaring and enforcing a scope's operating mode. Every
kit-using CLI (tlc, wsm, aps, pod, ctxt) reads the mode and optionally
enforces it. Mode lives on the existing `core/projects` registry entry;
enforcement rides on `runtime/policy` via the default
...
`StageActive`, so this is zero-touch for adopters not opting in.
...
- [`go/core/stage/README.md`](../../../go/core/stage/README.md): package README
- [`runtime/policy/stage.yaml`](../../../go/runtime/policy/stage.yaml): default rule set
