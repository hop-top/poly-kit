# qmochi Chart Gallery

> Every chart type qmochi renders, with its options, its worked example
> and its output: bar, column, line, sparkline, heatmap, scatter,
> braille and pie, plus SVG output and automatic chart selection. The
> package front page is
> [`incubator/qmochi/README.md`](../../../incubator/qmochi/README.md),
> which carries install, the chart-type table and the style matrix.

## Who this is for

Go authors who already have `hop.top/qmochi` and are picking a chart
type or reaching for one of its options.
