# Process State API Reference

> Reference for `hop.top/kit/go/console/ps`: process state monitoring
> and management for hop.top CLI tools. The package gives every adopter
> a consistent `<tool> ps` subcommand plus the supervisory primitives
> needed to spawn, watch, and stop the child processes that subcommand
> reports on.
...
| `StdioMode`     | Effect                                              |
|-----------------|-----------------------------------------------------|
| `StdioInherit`  | Default. Leaves `cmd.Stdout/Stderr` as the caller configured them. |
...
- [`go-primitives.md`](go-primitives.md): where `ps` sits among the Go primitives
- [`cli-api-reference.md`](cli-api-reference.md): the root the `ps` subcommand mounts on
