...
> Opt-in, redact-before-egress CLI telemetry for kit-based tools
> (`hop.top/kit/go/runtime/telemetry`). Intent table, mode contract, field
> presence, identity, HTTPS sink and spool, build-time options, consent
> hook, wire format, concurrency notes.

## Who this is for

Go authors who want to know how their command surface is actually used
without exfiltrating user data.

Three tiers: `ModeOff` (default, zero-cost no-op), `ModeAnon` (install_id,
command, exit, duration) and `ModeFull` (adds args and flags AFTER redact).
Emission additionally requires a granted `ConsentHook`; the default hook
denies. stdout and stderr are NEVER captured at any tier.

The package supplies a `Mode` gate, an anonymous rotatable
`installation_id`, a `ConsentHook` seam, and a batched HTTPS sink with
on-disk spool fallback. Adopters get one config knob, `Mode`, that maps to
the three-tier privacy contract. The schema is mirrored by polyglot SDKs
and a cross-language contract test diffs the wire shape.
...
The package is polyglot-aware: the on-disk `installation_id` lives at a
fixed path so the Python, TS, Rust and PHP SDK siblings read the same
identifier without re-hashing. The wire `Event` shape is part of the
contract too: JSON tags, field order and `omitempty` placement are diffed
by the cross-language contract test.

## Intent table

| Adopter intent                                              | API                                                          |
|-------------------------------------------------------------|--------------------------------------------------------------|
| I want telemetry off, what is the default?                   | `ModeOff` (no-op; nothing to do)                             |
...
- `var DefaultEndpoint string` is the ldflag target. Adopter release
...
[`cli.TelemetryConfig`](../../../go/console/cli/telemetry.go) is the
...
[Telemetry compliance, section 7](telemetry-compliance.md#7-build-time-configuration-kit-options)
explains the GitHub Actions pattern for adopters; this section is the
engineer-level mirror.
...
[`hop.top/kit/go/core/consent`](../../../go/core/consent/), which installs
the persisted-decision reader at `cobra.OnInitialize` via
`SetConsentHook(...)`.
...
emit-gate decision. The richer Decision value object (state plus source)
lives inside `go/core/consent` and never crosses the emitter boundary.
...
Audit topic: `kit.telemetry.redact.matched`. A compliance subscriber uses
it to verify redact actually fired before any Full
...
- `schema_version` is a **string** (`"1"`), not an int, because weakly-typed
...
- `globalMode` is an `atomic.Int32`; `appPrefix`, `globalHook`, and
  ...
  (`hookHolder`, `observerBox`) because `atomic.Value` panics on
...
- [`go/runtime/telemetry/README.md`](../../../go/runtime/telemetry/README.md): package README
- [telemetry-compliance.md](telemetry-compliance.md): the F13 `ConsentingTelemetry` checklist
- [`go/core/consent/`](../../../go/core/consent/): persisted decision, resolver, and the `consent.NewHook(...)` that satisfies `ConsentHook`
- [`go/core/redact/`](../../../go/core/redact/): redact rule loading; `MustLoadRedactor()` is the kit-blessed entry point
- [`go/runtime/bus/`](../../../go/runtime/bus/): topic grammar validator, `bus.NewEvent`, `bus.Sink`
- [`go/runtime/provenance/`](../../../go/runtime/provenance/): the `Mode` plus atomic plus one-shot env-read idiom this package mirrors
