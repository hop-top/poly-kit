...
> Wire-level provenance for kit-based tools
> (`hop.top/kit/go/runtime/provenance`). Wrapper choice, adopter happy
> path, strict modes, envelope shape, lint, source wrappers, harness
> primitives, cassette cross-check, v1 limitations.

## Who this is for

Go authors emitting structured output whose readers must answer "where did
this number come from?" without grepping source or replaying cassettes.
Factor 12 (Hops First-CLI Conformance Conventions) requires every
Synthesized or Cached value that flows into structured output to carry
metadata describing its origin.

Two parametrised wrappers, `Cached[T]` and `Synthesized[T]`, mark the
non-authoritative provenance of a single field. A per-context `Tracker`
records the metadata keyed by RFC 6901 JSON pointer. The `Render` boundary
...
| `ModeStrict` | Refuses to emit; returns `*output.Error{Code: "PROVENANCE_MISSING", ExitCode: 65}`. | Production, where silent provenance regressions become loud. |
...
structural, flagging missing JSON tags and zero-value literals at
...
The conformance harness re-exports these under its own name; the
implementation lives here because this package owns the wire contract.
...
- **No attestation / signing.** This package guards against honest
  mistakes and capability drift, not adversarial authors. See survey sections 1 and 8 R6.
...
- [`go/runtime/provenance/README.md`](../../../go/runtime/provenance/README.md): package README
