...
> Runtime circuit breakers for kit-based tools (`hop.top/kit/go/core/breaker`).
> Policy table, API surface, wrap helpers, examples, bus topics,
> `breaker.yaml` schema, `kit breaker` subcommands, limitations.

## Who this is for

Go authors bounding the runtime blast radius of any operation — file
writes, exec spawns, HTTP calls, token spend — that path policy alone
(`kit/scope`) cannot catch. Where `kit/scope` answers *where can I
touch?*, `kit/breaker` answers *how much, how fast, how often before I
stop?*.

Thin wrapper over [failsafe-go][failsafe] (MIT). The two policies
failsafe doesn't ship — `MaxBytes` (Volume) and `MaxOps` (Count) —
are implemented natively under
...
- [`go/core/breaker/README.md`](../../../go/core/breaker/README.md): package README
- [scope.md](scope.md): path policy, the sibling guardrail
