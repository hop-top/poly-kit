...
> Per-user directory and file resolution for kit-based tools
> (`hop.top/kit/go/core/xdg`). API surface, examples, concurrency
> guarantees and their cost, platform notes.

## Who this is for

Go authors resolving where a tool's config, data, cache, state and runtime
files live, and agents that need the user's own Documents, Downloads and
similar directories.
...
`name` may include subdirs (`"sub/app.yaml"`); they are created on demand.

### Search functions: user dir plus system XDG\_\*\_DIRS
...
Returns the first existing match. Lets users and ops drop org-wide defaults
under, for example, `/etc/xdg/<tool>/`.
...
and rewrites all of them on each refresh, holding no lock, so concurrent
resolves through it directly are a data race. This package serializes the
refresh and the read that follows it, which is what makes the wrapper safe
...
- **macOS**: `Config*` uses `~/Library/Preferences`; `Data*` and `State*`
  use `~/Library/Application Support` per Apple HIG. `XDG_*_HOME` overrides
  them (kit and adrg/xdg both respect it).
...
Full per-platform path table: see
[adrg/xdg README](https://github.com/adrg/xdg#default-locations).

## Related pages

- [`go/core/xdg/README.md`](../../../go/core/xdg/README.md): package README
- [config.md](config.md): the config loader that resolves through these paths
- [scope.md](scope.md): path policy; `xdg.SetGuard` wires the xdg to scope hook
